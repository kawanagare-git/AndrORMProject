...
Unit tests (Runtime / Common / KSP / Detekt / shared-library):
...
An additional task is provided to analyze only Android tests in the `app` module.
...
> [!NOTE]
> When a SELECT entity is generated by KSP, AndrORM also generates an entity-specific Cursor conversion routine, allowing `executeSelectAsEntityList()` to convert directly from the Cursor to the entity.
>
> When a SELECT entity is defined manually, this KSP-generated conversion routine does not exist, so `executeSelectAsEntityList()` falls back to the conventional entity-conversion path that uses a Map and reflection.
>
> When multiple SELECT entities are used in a JOIN or similar query, if any one of the entities does not have a KSP-generated Cursor conversion routine, the entire SELECT falls back to the conventional entity-conversion path.
>
> This difference concerns only the internal processing and execution efficiency of entity retrieval. It does not change which features can be used with the SQL builders or the specification of the returned results.
...
| Category | Framework |
|---|---|
| Unit Test | Executed in the Runtime, Common, KSP, Detekt, and shared-library modules |
| Android Test | JUnit4 |
