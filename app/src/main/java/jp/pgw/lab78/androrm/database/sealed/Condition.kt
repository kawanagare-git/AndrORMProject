...
sealed class Compare : Condition() {
     ...
     * ## 結合条件記述用クラス
     * ### JOIN に使用する単一条件を指定
     * @param mainProperty 検索条件のカラム
     * @param operator 検索演算子
     * @param joinedProperty 検索条件のカラム
     */
...
data class LogicalCondition(
    ...
    companion object {
        fun and(vararg cond: Condition): LogicalCondition =
            LogicalCondition("AND", cond.toList())

        fun or(vararg cond: Condition): LogicalCondition =
            LogicalCondition("OR", cond.toList())
    }
}